...
IN_PATH = os.path.join(os.path.dirname(__file__), "in", "Weapons_Resize_34.jpg")
ICONS_BASE = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "icons")
TARGET_SIZE = (256, 256)

# Batch 34 — rest of the armor catalog. All filenames are exact ARMOR_MAP ids —
# self-wiring, buildArmorItemData() already points there.
SLOTS = [
    ("plate-carrier-iii",              "armor"),  # 1
    ("pancerz-kompozytowy",            "armor"),  # 2
    ("ciezka-zbroja-smieciowa",        "armor"),  # 3
    ("pelna-zbroja-smieciowa",         "armor"),  # 4
    ("pancerz-stalowej-policji",       "armor"),  # 5
    ("wojskowy-pancerz-hydrauliczny",  "armor"),  # 6
    ("helm",                           "armor"),  # 7
    ("ochraniacze-nog",                "armor"),  # 8
    ("ochraniacze-rak",                "armor"),  # 9
]
