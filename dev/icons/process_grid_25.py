...
IN_PATH = os.path.join(os.path.dirname(__file__), "in", "Weapons_Resize_25.png")
ICONS_BASE = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "icons")
TARGET_SIZE = (256, 256)

SLOTS = [
    ("papieros",                 "items/drugs"),  # 1: cigarette — Papieros
    ("tyton_do_zucia",           "items/drugs"),  # 2: chewing tobacco — Tytoń do żucia
    ("nitrogliceryna",           "items/loot"),   # 3: nitroglycerine — Nitrogliceryna
    ("chemia",                   "items/loot"),   # 4: chemicals collection — Chemia (CH)
    ("materialy_organiczne",     "items/loot"),   # 5: animal parts — Materiały Organiczne (MO)
    ("czesci",                   "items/loot"),   # 6: mechanical parts — Części zapasowe (CZ)
    ("czesci_elektroniczne",     "items/loot"),   # 7: electronic parts — Części elektroniczne (CE)
    ("materialy_konstrukcyjne",  "items/loot"),   # 8: construction materials — Materiały konstrukcyjne (MK)
    ("woda_filtrowana",          "items/loot"),   # 9: filtered water — Woda filtrowana (1l)
]
