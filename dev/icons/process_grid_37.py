...
IN_PATH = os.path.join(os.path.dirname(__file__), "in", "Weapons_Resize_37.jpg")
ICONS_BASE = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "icons")
TARGET_SIZE = (256, 256)

# Batch 37 — Alan's 5 remaining flavour placeholders (no catalog home, pure
# narrative dressing) plus 4 shared starter-kit items that recur across
# Buźka/Carson/Dante/Góra/Iris/Kluczyk and were still on icons/svg/item-bag.svg.
SLOTS = [
    ("nomeksowy_kombinezon", "items/loot"),  # 1: Nomex coverall (Alan)
    ("ceramiczny_kubek",     "items/loot"),  # 2: ceramic mug (Alan)
    ("okulary",              "items/loot"),  # 3: reading glasses (Alan)
    ("dlugopis",             "items/loot"),  # 4: pen (Alan)
    ("identyfikator",        "items/loot"),  # 5: ID card (Alan)
    ("spiwor",               "items/loot"),  # 6: Śpiwór (shared starter kit)
    ("menazka",              "items/loot"),  # 7: Menażka (shared starter kit)
    ("ubranie",              "items/loot"),  # 8: Ubranie (shared starter kit)
    ("zestaw_zolnierza",     "items/loot"),  # 9: Zestaw Żołnierza (Lorentz)
]
