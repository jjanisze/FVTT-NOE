...
IN_PATH = os.path.join(os.path.dirname(__file__), "in", "Weapons_Resize_22.png")
ICONS_BASE = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "icons")
TARGET_SIZE = (256, 256)

# (filename_stem, subdirectory_under_icons/) or None to skip a grid cell.
# Grid is 3x3, read left-to-right, top-to-bottom.
SLOTS = [
    ("activity_mag_swap", "activities"),   # 1 (0,0): magazine swap activity icon
    ("whip",              "weapons"),       # 2 (0,1): whip — for 'Bat' weapon
    None,                                   # 3 (0,2): baseball bat — skipped, old icon reused for Bejsbol
    ("taurus",            "items/drugs"),   # 4 (1,0): 250 ml can — Taurus
    ("trybiotyl",         "items/drugs"),   # 5 (1,1): ointment tube — Trybiotyl
    ("radoff",            "items/drugs"),   # 6 (1,2): intravenous bag — RadOff
    ("radmov",            "items/drugs"),   # 7 (2,0): iodine/KI tablets — RadMov
    ("proch_strzelniczy", "items/loot"),    # 8 (2,1): good gunpowder — Proch strzelniczy
    ("proch_czarny",      "items/loot"),    # 9 (2,2): bad gunpowder — Proch czarny
]
