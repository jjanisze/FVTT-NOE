...
IN_PATH = os.path.join(os.path.dirname(__file__), "in", "Weapons_Resize_28.png")
ICONS_BASE = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "icons")
TARGET_SIZE = (256, 256)

# Third batch — last 4 toolkits + 5 general-use spare items (3x3 grid, row-major).
SLOTS = [
    ("szulera",     "tools"),       # 1: Mały szuler
    ("szklarza",    "tools"),       # 2: Mały szklarz
    ("slusarza",    "tools"),       # 3: Mały ślusarz
    ("tatuazysty",  "tools"),       # 4: Mały tatuażysta
    ("notatka",     "items/loot"),  # 5: Kawałek papieru z notatką
    ("pudelko",     "items/loot"),  # 6: Małe pudełko
    ("szkatulka",   "items/loot"),  # 7: Szkatułka
    ("dvd",         "items/loot"),  # 8: Dysk DVD z danymi
    ("plytka",      "items/loot"),  # 9: Elektroniczna płytka
]
