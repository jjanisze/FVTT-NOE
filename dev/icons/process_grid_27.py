...
IN_PATH = os.path.join(os.path.dirname(__file__), "in", "Weapons_Resize_27.png")
ICONS_BASE = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "icons")
TARGET_SIZE = (256, 256)

# Toolkit icons — second batch (3x3 grid, row-major).
# Names MUST match toolkit ids in scripts/config/toolkits-data.mjs (icons/tools/<id>.svg).
SLOTS = [
    ("klusownika",  "tools"),  # 1: Mały kłusownik
    ("kowala",      "tools"),  # 2: Mały kowal
    ("krawca",      "tools"),  # 3: Mały krawiec
    ("kucharza",    "tools"),  # 4: Mały kucharz
    ("mechanika",   "tools"),  # 5: Mały mechanik
    ("medyka",      "tools"),  # 6: Mały medyk
    ("rusznikarza", "tools"),  # 7: Mały rusznikarz
    ("rzeznika",    "tools"),  # 8: Mały rzeźnik
    ("stolarza",    "tools"),  # 9: Mały stolarz
]
