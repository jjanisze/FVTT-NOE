...
IN_PATH = os.path.join(os.path.dirname(__file__), "in", "Weapons_Resize_35.jpg")
ICONS_BASE = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "icons")
TARGET_SIZE = (256, 256)

# Batch 35 — Tarcza (last ARMOR_MAP entry, self-wiring) + 8 of the 12 crafting
# gear-placeholder stubs (gear-data.mjs GEAR_PLACEHOLDERS). The placeholders
# need a matching code change (buildGearItemData) to actually pick these up —
# see the follow-up edit to gear-data.mjs.
SLOTS = [
    ("tarcza",           "armor"),       # 1: Tarcza (ARMOR_MAP id)
    ("belty",            "items/loot"),  # 2: Bełty
    ("igly",             "items/loot"),  # 3: Igły
    ("klodka",           "items/loot"),  # 4: Kłódka
    ("kolczatki",        "items/loot"),  # 5: Kolczatki
    ("lom",              "items/loot"),  # 6: Łom
    ("lopata",           "items/loot"),  # 7: Łopata
    ("podkowy",          "items/loot"),  # 8: Podkowy
    ("plyty_pancerne",   "items/loot"),  # 9: Płyty pancerne
]
