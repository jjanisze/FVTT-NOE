...
IN_PATH = os.path.join(os.path.dirname(__file__), "in", "Weapons_Resize_23.png")
ICONS_BASE = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "icons")
TARGET_SIZE = (256, 256)

# (filename_stem, subdirectory_under_icons/) or None to skip.
SLOTS = [
    ("wd_tabs",      "items/drugs"),   # 1: water treatment tablets — WD-Tabs
    ("painkiller",   "items/drugs"),   # 2: painkiller capsules — Painkiller
    ("neuro_cola",   "items/drugs"),   # 3: medicinal cola — Neuro-Cola
    ("medpak",       "items/drugs"),   # 4: medpak — Medpak
    ("detoks",       "items/drugs"),   # 5: antivenom kit — Detoks
    ("iv_bag_spare", "items/drugs"),   # 6: intravenous bag spare (no world item yet)
    ("deadline",     "items/drugs"),   # 7: big fancy syringe — Deadline
    ("ar35_beta",    "items/drugs"),   # 8: military peptide injector — AR-35 BETA
    ("ar23",         "items/drugs"),   # 9: civilian peptide applicator — AR-23
]
