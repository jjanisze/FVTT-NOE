...
IN_PATH = os.path.join(os.path.dirname(__file__), "in", "Weapons_Resize_29.jpg")
ICONS_BASE = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "icons")
TARGET_SIZE = (256, 256)

# Batch 29 — Pochodnia (3 states) + Piekarz inventory backlog (3x3 grid, row-major).
SLOTS = [
    ("pochodnia_improwizowana", "weapons"),      # 1: Pochodnia Improwizowana
    ("pochodnia_smolowa",       "weapons"),      # 2: Pochodnia Smołowa
    ("pochodnia_wypalona",      "weapons"),      # 3: Wypalona Pochodnia
    ("fajka",                   "items/loot"),   # 4: Fajka
    ("nabicie_tytoniu",         "items/loot"),   # 5: Nabicie tytoniu
    ("manierka",                "items/loot"),   # 6: Manierka
    ("zdjecie_kobiety",         "items/loot"),   # 7: Powojenne zdjęcie kobiety
    ("flaga_usa",               "items/loot"),   # 8: FlagaUSA
    ("zetony",                  "items/loot"),   # 9: Żetony
]
