...
const RULES = [
    ...
    { keys: ["mina przeciwpiechotna", "mine antipersonnel", "anti-personnel mine", "anti-personnell mine"], icon: "weapons/anti_personnel_mine.svg" },
    { keys: ["mina przeciwpojazdowa", "mine antivehicle", "anti-vehicle mine"], icon: "weapons/anti_vehicle_mine.svg" },
    { keys: ["pipebomb", "pipe bomb", "bomba rurowa", "lont"], icon: "weapons/pipebomb_fuze.svg" },
    { keys: ["dynamit", "dynamite", "zdalny detonator", "remote detonator"], icon: "weapons/dynamite_remote_bundle.svg" },
    { keys: ["c4", "plastik", "plastic explosive", "ładunek c4", "ladunek c4"], icon: "weapons/c4_remote_charge.svg" },
...
export function setupWeaponIcons() {
    Hooks.on("preCreateItem", (item, data, options, userId) => {
        // Accept weapons, but also consumables and loot (for ammo and explosives)
        if (!["weapon", "consumable", "loot", "equipment"].includes(item.type)) return;
        ...
        // Always try to replace placeholder/avatar icons for recognized weapons
        if (targetIcon && item.img !== targetIcon) {
            item.updateSource({ img: targetIcon });
        }
    });
}
